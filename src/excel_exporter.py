...
and returns the raw bytes, ready to be uploaded to Drive or attached to an email.
...
def build_monthly_excel(invoices: list[dict], year: int, month: int) -> bytes:
    ...
    # Data rows
    # -----------------------------------------------------------------------
    # Track per-supplier totals (keyed by supplier display name)
    ...
    summary_header_labels = ["Fournisseur", "Nb factures", "Total HT", "Total TVA", "Total TTC"]
    for col_idx, label in enumerate(summary_header_labels, start=1):
    ...
    for col_idx, val in enumerate(total_labels, start=1):
    ...
    # -----------------------------------------------------------------------
    # Column widths (based on invoice data rows only)
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # Freeze header + auto-filter on data rows only
    # -----------------------------------------------------------------------
    ws.freeze_panes = "A2"
    last_data_col = get_column_letter(len(COLUMNS))
    ws.auto_filter.ref = f"A1:{last_data_col}{len(invoices) + 1}"

...
class GraphClient:
    ...
    def send_mail(self, payload: dict) -> None:
        """Send an email via Graph API (from the authenticated user's account)."""
        ...
        for attempt in range(2):
            ...
            resp.raise_for_status()
            return
        raise RuntimeError("sendMail failed after token refresh")

    def create_draft(self, message: dict) -> str:
        """
        Save a message as a draft in the authenticated user's mailbox.

        Args:
            message: The Graph API message object (same shape as payload["message"]
                     used in sendMail, i.e. subject, body, toRecipients, attachments…).

        Returns:
            The draft message id (can be used to open it in Outlook).
        """
        url = f"{GRAPH_BASE}/me/messages"
        for attempt in range(2):
            resp = requests.post(
                url, headers=self._headers(), json=message, timeout=60
            )
            if resp.status_code == 401 and attempt == 0:
                logger.warning("Token expired, refreshing...")
                self._refresh_token()
            ...
            return draft_id
        raise RuntimeError("create_draft failed after token refresh")
